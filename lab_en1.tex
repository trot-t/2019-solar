...
switch $S4$ to the appropriate position. The SPD under review (thyristor $T$) is connected to terminals $X1-X4$.

The voltage and current of the power circuit of the SPP are regulated by the $AT$ autotransformer. 
...
are determined by a voltmeter $V3$ and an ammeter $A3$.

The power supply circuit of the SPD under the studying of its characteristics 
in the open state is carried out through transformer $Tr2$. 
...
is provided by the $D5$ diode
and the sinusoidality of the current curve is achieved by the diodes $D6$, $D7$ 
(creating a circuit for the flow of current
in the second (non-working) half-period of the transformer voltage $Tr2$ and excluding due to this bias
...
The average values of voltage and current of the SPD are determined by a $V2$ voltmeter
and ammeter $A2$. To observe the curves of the anode voltage and current, 
...
transformer $Tr1$. 
The average values of the voltage and current of SPD are measured with a $V1$ voltmeter and a $A1$ ammeter.
...
In this case, to the SPD through the diodes $D1$, $D2$, $DЗ$ is supplied
half-wave voltage from the secondary winding of the transformer $Tr1$. 
The $D4$ diode sharply reduces the value
negative half-wave voltage applied to the diode $D3$ and SPD, which ensures the flow of current
...
To observe the static volt-ampere characteristics of the SPD in the closed state, the $S2$ switch connects
...
\item set the autotransformer $AT$ to the lower position;
...
\item Observe and draw the static volt-ampere characteristic of the thyristor's control circuit 
$I_{ctrl} = f(U_{ctrl})$.
\item Determine the values of the control current $I_{ctrl.O}$ and the control voltage $U_{ctrl.O}$ wich open the thyristor.
\item Observe and draw a direct branch of the classification volt-ampere characteristic of the thyristor 
$I_a = f(U_a)$ in the open state.


Sketch from the oscilloscope screen a direct branch of the dynamic volt-ampere characteristic of the thyristor
in the open state $I_a=f(U_a)$ and the curves of the anode current $I_a=f(\omega t)$ 
and voltage $U_a = f(\omega t)$ at the maximum anode current.
...
volt-ampere characteristics of the thyristor in the closed state at the following current values
...
\begin{enumerate}
\item The static volt-ampere characteristic of the control circuit of the SPD is observed on a direct current
...
\item Classification volt-ampere characteristics of the power circuit of the SPD are observed at average values
...
\item The observation of the direct branch of the dynamic volt-ampere characteristics of the SPD 
...
\item To avoid damaging of the device when taking the volt-ampere characteristics in the closed state
...
\begin{enumerate}
\item according to the volt-ampere characteristic in the low conductivity state taken at $I_{ctrl} = 0$, 
...
the control current is set equal to $1.5 I_{ctrl\:\:o}$; by adjusting the autotransformer $AT$, 
